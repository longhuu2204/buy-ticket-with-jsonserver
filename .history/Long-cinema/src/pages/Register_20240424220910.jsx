...
const RegisterSchema = Yup.object().shape({
  ...
  password: Yup.string().required("Bắt buộc!").min(6, "Mật khẩu phải có ít nhất 6 ký tự"),
...
const Register = () => {
  ...
  const checkExistingEmail = async (email) => {
    try {
      const response = await axios.get(`http://localhost:3000/account?email=${email}`);
          ...
          <div className="lg:w-2/6 xl:w-2/5 md:w-2/3 bg-gray-100 rounded-lg p-8 flex flex-col lg:ml-auto w-full mt-10 lg:mt-0">
            ...
            <div className="relative mb-4">
              ...
              <Field
                ...
                className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
              />
              <a className="errors text-sm text
